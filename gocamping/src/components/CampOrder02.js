...
class CampOrder02 extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      foodname: props.order.camplist_name,
      price: props.order.camplist_price,
      total: props.order.camplist_price * + props.orderlist_number,
      camplistData: props.order,
      orderlist_total:props.order.camplist_price,
      date:props.date,
      day:props.day,
      sum:props.sum,
      num:props.numtotal,
 ...
 componentWillReceiveProps(nextProps) {
    this.setState({
      camplistData: nextProps.data,
      price: nextProps.order.camplist_price,
      total: nextProps.order.camplist_price,
      date: nextProps.date,
      day: nextProps.day,
      sum: nextProps.sum,
      num:nextProps.numtotal,
    });
  }
  ...
  render() {
    ...
    console.log(this.props.order.date)
    console.log(this.props.date)
    let {date} = this.props
    let {day} = this.props
    let {sum} = this.props
    let {num} = this.props
    let{orderlist_total}=this.props
    return (
      <>
        {/* 進入訂單第一步 訂單明細*/}
        <Container className="mt-1">
          <Row className="mt-5">
            <Col>
              <div className="title fs-32">訂購明細</div>
            </Col>
          </Row>
        </Container>
        {/* 訂購清單 */}
        <Container className="mt-1">
          <Row className="mt-3">
            <Col>
              <div className="grass fs-24">訂購清單</div>
            </Col>
          </Row>
        </Container>
        ...
        <Container className="mt-1">
          <Row className="mt-3">
            ...
            <Image
                        style={{ width: "100%" }}
                        src={this.props.order.camplist_image}
                        rounded
                      />
            ...
            <Col sm={6}>
            <p className="grass">
                <FaMapMarkerAlt /> {this.props.order.camplist_cityname}{this.props.order.camplist_distname}
              </p>
              <p className="fs-24"> {this.props.order.camplist_name}</p>
              <div>
                <div className="d-flex my-3">                 
                  <p className="mr-5">入住日期</p>
                  <p >{date}</p>
                </div>
                <div className="d-flex my-3">                 
                  <p className="mr-5">入住天數</p>
                  <p>{day}晚</p>
                </div>
                <div className="d-flex my-3">
                   <p className="mr-5">營位數量</p>
                    <p>{num}帳</p>  
                </div>
                <div className="d-flex my-3">
                  <div className="mr-5">金額總計 </div>
                  <p>{sum}元</p>
                </div>
              </div>
        ...
        <Container>
          <Row className="mt-3">
            <Col style={{ width: "100%" }}>
            <h3 className="fs-24 grass">注意事項</h3>
              <div className="m-3">
              <p className="ground">
                『營地預訂』、『食材預購』與『活動報名』皆是在各自頁面進行結帳喔！
              </p>
              </div>
            </Col>
          </Row>
        ...
        <Container className="mt-1">
          <Row className="mt-3">
            <Col>
              <div className="grass fs-24">服務條款</div>
            </Col>
          </Row>
        </Container>
        <Container>
          <Row >
            <Col>
              <div style={{ width: "100%" }}>
                <div className="service border fs-14 p-2">
                  <p>
                    一、會員服務條款<br/>
            ...
            <Col>
              <ButtonToolbar className="justify-content-end">
                <Button
                  ...
                  style={{ width: "30%" }}
                  sm={"block"}
                  href={"/Camp/CampDetails/" + this.props.order.id}
                >
                  返回營地列表
                </Button>
                <Button
